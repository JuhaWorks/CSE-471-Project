...
const ProjectCard = memo(({ project, user, onlineUsers, toggleGlobalPresence, respondMutation, restoreMutation, purgeMutation, togglePinMutation, view, EASE, getStatusStyles }) => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={EASE} className="group relative flex flex-col h-full rounded-[1.25rem] hover:bg-white/[0.02] border border-transparent hover:border-white/5 transition-all p-1.5">
        <div className="relative mb-3 aspect-[16/10] overflow-hidden rounded-[1rem] border border-white/5">
            ...
            <div className="absolute top-2 left-2">
                <button 
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); togglePinMutation.mutate(project._id); }} 
                    className={twMerge(
                        "p-1.5 rounded-lg backdrop-blur-md border transition-all active:scale-90 group/pin", 
                        project.isPinned 
                            ? "bg-theme/20 border-theme/30 text-theme shadow-theme-sm" 
                            : "bg-white/5 border-white/10 text-tertiary hover:bg-white/10 hover:text-secondary"
                    )}
                >
                    <motion.div
                        animate={project.isPinned ? { scale: [1, 1.2, 1], rotate: [0, 15, 0] } : {}}
                        transition={{ duration: 0.4 }}
                    >
                        <Pin className={twMerge("w-3 h-3 transition-colors", project.isPinned && "fill-current")} />
                    </motion.div>
                </button>
            </div>
            <div className="absolute top-2 right-2 flex flex-col items-end gap-1.5">
                <div className={twMerge("px-1.5 py-0.5 rounded-md text-[7px] font-black uppercase tracking-widest border transition-all", getStatusStyles(project.status))}>{project.status || 'Active'}</div>
                {user?.interfacePrefs?.showGlobalPresence !== false && onlineUsers.filter(u => project.members?.some(m => (m.userId?._id || m.userId) === u.userId)).length > 0 && (
                    <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleGlobalPresence(true, project._id); }} className="flex items-center gap-1 px-1.5 py-0.5 bg-success/20 border border-success/30 rounded-md"><span className="w-1 h-1 rounded-full bg-success animate-pulse" /><span className="text-[7px] font-black text-success uppercase tracking-widest">Live</span></button>
                )}
            </div>
        </div>
        <div className="px-1.5 pb-2 flex-1 flex flex-col space-y-2">
            <div>
                <h3 className="text-base font-black text-primary group-hover:text-theme transition-colors line-clamp-1">{project.name}</h3>
                <p className="text-tertiary text-xs font-semibold line-clamp-1 opacity-70">{project.description}</p>
            </div>
            <div className="flex items-center justify-between pt-2 border-t border-white/5">
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1 opacity-40"><Calendar className="w-2.5 h-2.5" /><span className="text-[9px] font-mono">{new Date(project.endDate).toLocaleDateString([], { month: 'short', day: 'numeric' })}</span></div>
                </div>
                <div className="flex items-center gap-1.5">
                    {view === 'invitations' ? (
                        <>
                            <button onClick={() => respondMutation.mutate({ id: project._id, status: 'rejected' })} className="text-[9px] font-black uppercase text-tertiary">Decline</button>
                            <button onClick={() => respondMutation.mutate({ id: project._id, status: 'active' })} className="text-[9px] font-black uppercase text-theme px-2 py-0.5 bg-theme/10 rounded-md">Accept</button>
                        </>
                    ) : view === 'archived' ? (
                        <>
                            <button onClick={() => purgeMutation.mutate(project._id)} className="text-[9px] font-black uppercase text-tertiary">Delete</button>
                            <button onClick={() => restoreMutation.mutate(project._id)} className="text-[9px] font-black uppercase text-success px-2 py-0.5 bg-success/10 rounded-md">Restore</button>
                        </>
                    ) : (
                        <>
                            <Link to={`/projects/${project._id}/settings`} className="p-2 rounded-lg bg-white/[0.03] border border-white/5 text-tertiary hover:text-primary transition-colors"><Settings className="w-3 h-3" /></Link>
                            <Link to={`/tasks?project=${project._id}`} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[9px] font-black bg-theme/10 text-theme uppercase hover:bg-theme/20 transition-all">Enter <ChevronRight className="w-2.5 h-2.5" /></Link>
                        </>
                    )}
                </div>
            </div>
                    ...
                    <AnimatePresence mode="popLayout">
                        {[
                            ...
                            <section key={section.title} className="space-y-8">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 3xl:grid-cols-6 gap-6 px-1">
                                    {section.list.map(p => (
