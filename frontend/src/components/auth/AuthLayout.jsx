...
export const Brand = ({ rightSide }) => {
    return (
        <aside className={twMerge(clsx(
            ...
            >
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-accent-500 to-accent-600 flex items-center justify-center shadow-2xl shadow-accent-500/20">
            ...
            <div className="relative z-10">
                <motion.p 
                    ...
                    className="text-xs font-bold uppercase tracking-[0.3em] text-theme mb-6"
                >
                    Team Workspace
                </motion.p>
                <h2 className="text-6xl font-bold leading-[0.9] tracking-tighter text-primary mb-6">
                    Build great things,<br />
                    <Typewriter />
                </h2>
                <p className="text-lg text-secondary leading-relaxed max-w-sm mb-10">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {FEATURES.map((feat, i) => (
                        <motion.div 
                            ...
                            className="group flex items-center gap-4 p-4 rounded-3xl bg-surface border-default"
                        >
                            <div className="w-10 h-10 rounded-2xl bg-sunken border border-default flex items-center justify-center transition-all group-hover:scale-110 group-hover:bg-accent-muted group-hover:border-accent">
                                <feat.icon className="w-5 h-5 text-tertiary group-hover:text-theme" />
                            </div>
            ...
            <footer className="relative z-10 flex items-center gap-4">
                <div className="flex -space-x-3">
                    {[1, 2, 3, 4].map((i) => (
                        <div key={i} className="w-8 h-8 rounded-full border-2 border-default bg-surface flex items-center justify-center overflow-hidden">
                             <div className="w-full h-full bg-gradient-to-br from-accent-400/20 to-accent-600/20" />
                        </div>
                    ))}
                </div>
                <div className="text-sm font-bold text-tertiary">
                    Trusted by <span className="text-theme">2,400+</span> teams worldwide
                </div>
            </footer>
        </aside>
    );
        ...
        <div className="min-h-screen bg-base text-primary selection:bg-accent-glow overflow-hidden font-sans relative">
            ...
            <div className="fixed inset-0 z-0 bg-base">
                <InteractiveGridBackground 
                    className="w-full h-full opacity-[0.6] dark:opacity-[0.3]"
                    gridSize={70}
                />
            </div>
