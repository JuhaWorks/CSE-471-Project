    ...
    return (
        <Dialog.Root open={open} onOpenChange={onOpenChange}>
            <Dialog.Portal>
                ...
                <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-zinc-950 border border-white/[0.08] rounded-3xl shadow-2xl z-[70] overflow-hidden focus:outline-none animate-in zoom-in-95 duration-200">
                    ...
                    <div className="p-8">
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <Dialog.Close className="p-2 text-zinc-500 hover:text-white hover:bg-white/5 rounded-full transition-all">
                                <X className="w-5 h-5" />
                            </Dialog.Close>
                        </div>
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                            <AnimatePresence mode="wait">
                                {step === 1 && (
                                    ...
                                    >
                                        <div className="space-y-2">
                                            <label className="text-[13px] font-semibold text-zinc-400 ml-1">Project Name</label>
                                            <input
                                                {...register('name')}
                                                placeholder="e.g. Project Orion"
                                                className="w-full bg-white/[0.03] border border-white/[0.08] rounded-2xl px-4 py-3 text-white placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500/40 transition-all"
                                            />
                                            {errors.name && <p className="text-xs text-red-400 ml-1">{errors.name.message}</p>}
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-[13px] font-semibold text-zinc-400 ml-1">Description</label>
                                            <textarea
                                                {...register('description')}
                                                rows={3}
                                                placeholder="What are we building today?"
                                                className="w-full bg-white/[0.03] border border-white/[0.08] rounded-2xl px-4 py-3 text-white placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500/40 transition-all resize-none"
                                            />
                                ...
                                {step === 2 && (
                                    ...
                                    >
                                        <div className="space-y-2">
                                            <label className="text-[13px] font-semibold text-zinc-400 ml-1">Category</label>
                                ...
                                {step === 3 && (
                                    ...
                                    >
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-2">
                                                <label className="text-[13px] font-semibold text-zinc-400 ml-1">Start Date</label>
                                                <div className="relative">
                                                    <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                                                    <input
                                                        type="date"
                                                        {...register('startDate')}
                                                        className="w-full bg-white/[0.03] border border-white/[0.08] rounded-2xl pl-11 pr-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500/40 transition-all [color-scheme:dark]"
                                                    />
                                                </div>
                                                {errors.startDate && <p className="text-xs text-red-400 ml-1">{errors.startDate.message}</p>}
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-[13px] font-semibold text-zinc-400 ml-1">End Date</label>
                                                <div className="relative">
                                                    <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                                                    <input
                                                        type="date"
                            ...
                            <div className="flex items-center gap-3 pt-4">
                                {step > 1 && (
                                    <button
                                        type="button"
                                        onClick={prevStep}
                                        className="flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl bg-white/[0.03] border border-white/[0.08] text-white font-semibold hover:bg-white/5 transition-all outline-none"
                                    >
                                ...
                                {step < 3 ? (
                                    <button
                                        type="button"
                                        onClick={nextStep}
                                        className="flex-1 flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl bg-white text-zinc-950 font-bold hover:bg-zinc-200 transition-all outline-none shadow-lg shadow-white/10"
                                    >
                                        Continue
                                    ...
                                    <button
                                        ...
                                        className="flex-1 flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl bg-gradient-to-r from-violet-600 to-blue-600 text-white font-bold hover:from-violet-500 hover:to-blue-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed outline-none shadow-lg shadow-violet-500/20"
                                    >
