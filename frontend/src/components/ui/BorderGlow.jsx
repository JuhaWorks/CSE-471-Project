...
const BorderGlow = ({
  ...
  const getEdgeProximity = useCallback((el, x, y) => {
    ...
    // Convert distance from edge to a 0-1 proximity value (1 is at edge, 0 is at center/far away)
    // The larger the element, the more the glow should be constrained to the edges
    const maxDist = Math.max(width, height) / 2;
    return Math.max(1 - (minDist / (maxDist * 0.5)), 0);
  }, []);
